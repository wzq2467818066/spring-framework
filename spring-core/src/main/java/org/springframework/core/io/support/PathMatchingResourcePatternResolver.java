...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			...
			// 路径不包含通配符
				...
				//，我们知道 #findAllClassPathResources(...) 方法，其实就是利用 ClassLoader 来加载指定路径下的资源，
				// 不论它是在 class 路径下还是在 jar 包中。如果我们传入的路径为空或者 /，
	...
	protected Set<Resource> doFindAllClassPathResources(String path) throws IOException {
		...
		//cl.getResources(path) 如果当前父类加载器不为 null ，则通过父类向上迭代获取资源，否则调用 #getBootstrapResources() 。
		...
		if ("".equals(path)) {
			...
			//若 path 为空（“”）时，则调用 #addAllClassLoaderJarRoots(...)方法。该方法主要是加载路径下得所有 jar 包，方法较长也没有什么实际意义
	 ...
	 * 在这个方法里面，我们要关注两个方法，
	 * 一个是 #determineRootDir(String location) 方法，
	 * 一个是 #doFindPathMatchingXXXResources(...) 等方法。
